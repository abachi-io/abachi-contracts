...
describe.only("Bond Depository", async () => {
    ...
    // Initial mint for Frax, ABI and DAI (10,000,000)
    const initialMint = "10000000000000000000000000";
    const initialABI = "10000000000000000";
    ...
    before(async () => {
        ...
        gAbiFactory = await smock.mock("MockGAbi");

        depositoryFactory = await ethers.getContractFactory("AbachiBondDepositoryV2");
    });
    ...
    beforeEach(async () => {
        ...
        abi = await erc20Factory.deploy("Abachi", "ABI", 9);
        treasury = await smock.fake("ITreasury");
        gABI = await gAbiFactory.deploy("50000000000"); // Set index as 50
        staking = await smock.fake("IStaking");
        depository = await depositoryFactory.deploy(auth.address, abi.address, gABI.address, staking.address, treasury.address);
        ...
        await abi.mint(deployer.address, "10000000000000")
        await treasury.baseSupply.returns(await abi.totalSupply());

        // Mint enough gABI to payout rewards
        await gABI.mint(depository.address, "1000000000000000000000")

        await abi.connect(alice).approve(depository.address, LARGE_APPROVAL);
    ...
    it("should not redeem before vested", async () => {
        let balance = await abi.balanceOf(bob.address);
        let amount = "10000000000000000000000"; // 10,000
        await depository.connect(bob).deposit(
            bid,
            amount,
        ...
        expect(await abi.balanceOf(bob.address)).to.equal(balance);
    });
    ...
    it("should redeem after vested", async () => {
        ...
        const bobBalance = Number(await gABI.balanceOf(bob.address));
        expect(bobBalance).to.greaterThanOrEqual(Number(await gABI.balanceTo(expectedPayout)));
        expect(bobBalance).to.lessThan(Number(await gABI.balanceTo(expectedPayout * 1.0001)));
    });

    it("should give correct rewards to referrer and dao", async () => {
        let daoBalance = await abi.balanceOf(deployer.address);
        let refBalance = await abi.balanceOf(carol.address);
        let amount = "10000000000000000000000"; // 10,000
        ...
        // Mint abi for depository to payout reward
        await abi.mint(depository.address, "1000000000000000000000");

        let daoExpected = Number(daoBalance) + Number(Number(payout) * daoReward / 1e4);
        await depository.getReward();

        const frontendReward = Number(await abi.balanceOf(deployer.address));
        ...
        const carolReward = Number(await abi.balanceOf(carol.address));
